...
uniform vec3 origin; //from which offset should be used
...
uniform mat4 projection, projview, model;
...
void emit_vertex(vec4 vertex, vec2 uv)
{
    ...
    // f_stroke_color    = stroke_color[0];
    // f_glow_color      = glow_color[0];
    EmitVertex();
}
...
void main(void)
{
    ...
    trans = (billboard ? projection : projview * qmat(g_rotation[0])) * trans;

    // Compute centre of billboard in clipping coordinates
    // vec4 vclip = projview*model*vec4(origin, 1) + trans*vec4(sprite_bbox_centre,0,0);
    vec4 vclip = projview*model*vec4(origin, 1) + trans*vec4(sprite_bbox_centre,0,0);
    ...
    // Compute required amount of bufferin
