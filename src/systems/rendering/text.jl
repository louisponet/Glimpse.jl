using FreeTypeAbstraction: FTFont, FT_Get_Char_Index, hadvance, leftinkbound, inkwidth,
                           get_extent, ascender, descender
...
function TextureAtlas(initial_size = TEXTURE_RESOLUTION[])
    return TextureAtlas(RectanglePacker(Rect2D(0, 0, initial_size...)),
                        Dict{Tuple{Char,String},Int}(), 1, zeros(Float16, initial_size...),
                        Vec4f0[])
end

function get_cache_path()
    return abspath(asset_path("fonts", ".cache",
...
function alternative_fonts()
    if isempty(_alternative_fonts)
        append!(_alternative_fonts,
                FTFont.(asset_path.(("fonts",), searchdir(asset_path("fonts")))))
    end
        ...
        try
            return open(get_cache_path()) do io
                ...
                fields = map(fieldnames(TextureAtlas)) do n
                    v = dict[n]
                    return isa(v, Vector) ? copy(v) : v # otherwise there seems to be a problem with resizing
                end
...
function to_cache(atlas)
    ...
    open(get_cache_path(), "w") do io
        dict = Dict(map(fieldnames(typeof(atlas))) do name
                        return name => getfield(atlas, name)
                    end)
        return Serialization.serialize(io, dict)
    end
end
...
function get_texture_atlas()
    if isassigned(global_texture_atlas) &&
       size(global_texture_atlas[]) == TEXTURE_RESOLUTION[]
...
function find_font_for_char(c::Char, font::FTFont)
    ...
    return error("Can't represent character $(c) with any fallback font nor $(font.family_name)!")
end
...
function render(atlas::TextureAtlas, glyph::Char, font, downsample = 5, pad = 6)
    ...
    uv == nothing &&
        error("texture atlas is too small. Resizing not implemented yet. Please file an issue at Makie if you encounter this") #TODO resize surface
...
function layout_text(string::AbstractString, textsize::Union{AbstractVector,Number}, font,
                     align, justification = 0.5, lineheight = 1.0)
    ...
    glyphpos = glyph_positions(string, font, textsize, offset_vec[1], offset_vec[2],
                               lineheight, justification)
...
function glyph_positions(str::AbstractString, font, fontscale_px, halign, valign,
    ...
    xs = map(extents) do extgroup
        return cumsum([isempty(extgroup) ? 0.0 : -leftinkbound(extgroup[1]);
                       hadvance.(extgroup[1:end-1])])
    end

    # each linewidth is the last origin plus inkwidth
    linewidths = last.(xs) .+
                 [isempty(extgroup) ? 0.0 : inkwidth(extgroup[end]) for extgroup in extents]
    maxwidth = maximum(linewidths)

    width_differences = maxwidth .- linewidths

    xs_justified = map(xs, width_differences) do xsgroup, wd
        return xsgroup .+ wd * justification
    end
    ...
    lineheights = map(char_groups) do group
        ...
        isempty(group) && return 0.0f0
        return Float32(font.height / font.units_per_EM * lineheight_factor * fontscale_px)
    end
...
function text_bb(str, font, size)
    positions = layout_text(str, Point2f0(0), size, font, Vec2f0(0),
                            Quaternionf0(0, 0, 0, 1), Mat4f0(I), 0.5, 1.0)
...
function get_iter(defaultfunc, dictlike, key)
    return make_iter(get(defaultfunc, dictlike, key))
end
...
function FontStorage()
    atlas = get_texture_atlas()
    fbo   = GLA.FrameBuffer(size(atlas.data), GLA.Texture(atlas.data; minfilter = :linear, magfilter = :linear, anisotropic = 16.0f0))
    return FontStorage(atlas, fbo)
end

@render_program TextProgram
function shaders(::Type{TextProgram})
    return [load_shader("sprites.geom"), load_shader("sprites.vert"),
...
function Overseer.update(::TextUploader, m::AbstractLedger)
    ...
    for e in @entities_in(text && spatial && ucolor)
        ...
        if !(e ∈ vao) || nsprites > length(vao[e])
            vao[e] = TextVao(VertexArray(generate_buffers(prog.program, GLint(0);
            ...
            GLA.upload!(vao[e]; color = fill(ucolor[e].color, nsprites),
                        rotation = fill(Vec4f0(0), nsprites), offset_width = offset_width,
                        uv_texture_bbox = uv_texture_bbox)
        end
        if !(e in vis)
            vis[e] = Visible()
...
function to_gl_text(string::AbstractString, textsize, font, align::Tuple{Symbol,Symbol},
                    atlas)
...
function Overseer.requested_components(::TextRenderer)
    return (Spatial, UniformColor, Camera3D, TextVao, Text, TextProgram, IOTarget,
            FontStorage)
end
...
function Overseer.update(::TextRenderer, m::AbstractLedger)
    ...
    gluniform(prog, :stroke_width, 0.0f0)
    gluniform(prog, :glow_width, 0.0f0)
