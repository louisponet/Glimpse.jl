...
nspheres = 20000
radii = Iterators.cycle(range(1.0f0, 200.0f0; length = div(nspheres, 100)))
...
for i in 1:nspheres
    color = cs[mod1(i, 2)]
    Gl.add_entity!(dio;
                   separate = [Gl.Spatial(; position = ps[i]), Gl.Material(), Gl.Shape(),
                               Gl.UniformColor(color), progtag],
                   shared = Gl.ComponentData[sph_geom])
